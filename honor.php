...
$query = $sql->query("SELECT characters.guid, characters.name, race, class, totalHonorPoints AS honor , totalKills AS kills, level, arenaPoints AS arena, guildid as GNAME, gender FROM `characters`,guild_member where race in (1,3,4,7,11) AND guild_member.guid = characters.guid ORDER BY $order_by DESC LIMIT 25;");
...
$query = $sql->query("SELECT characters.guid,characters.name,race,class, totalHonorPoints AS honor , totalKills AS kills, level, arenaPoints AS arena, guildid as GNAME, gender FROM `characters`,guild_member where race not in (1,3,4,7,11) AND guild_member.guid = characters.guid ORDER BY $order_by DESC LIMIT 25;");
$this_page = $sql->num_rows($query);
$output .= "<script type=\"text/javascript\">
answerbox.btn_ok='{$lang_global['yes_low']}';
answerbox.btn_cancel='{$lang_global['no']}';
