...
list($username,$pass,$gmlevel,$mail,$joindate,$last_ip,$failed_logins,$locked,$last_login,$expansion) = $data;
$sqlr->query("INSERT INTO account (username,sha_pass_hash,gmlevel,email, joindate,last_ip,failed_logins,locked,last_login,active_realm_id,expansion)
              VALUES (UPPER('$user_name'),'$pass',0,'$mail',now(),'$last_ip',0,$create_acc_locked,NULL,0,$expansion)");
				
$result = $sqlr->query("SELECT * 
							FROM account 
...
$sqlm->query("DELETE FROM mm_account_verification 
				WHERE username='$username'");
