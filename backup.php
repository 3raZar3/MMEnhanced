...
function dobackup()
{
  ...
  if (("load" == $backup_action)&&("file" == $backup_from_to))
  {
    ...
    if (file_exists("$backup_dir/$file_name_new"))
    {
      ...
      {
        foreach ($characters_db as $db)
          if ($use_db == $db['name']) $queries = run_sql_script($db['addr'], $db['user'], $db['pass'], $db['name'], "$backup_dir/$file_name_new",true);
      }
...
switch ($err)
{
  case 1:
    $output .= "
          <h1><font class=\"error\">{$lang_global['empty_fields']}</font></h1></div>";
    break;
  case 2:
    $output .= "
          <h1><font class=\"error\">{$lang_backup['backup_finished']}</font></h1></div>";
    break;
  case 3:
   $output .= "
          <h1>{$lang_backup['select_backup']}</h1></div>";
    break;
  case 4:
    if(isset($_GET['tot']))
      $total_queries = $_GET['tot'];
        ...
        $total_queries = NULL;
    $output .= "
          <h1><font class=\"error\">{$lang_backup['file_loaded']} $total_queries {$lang_backup['que_executed']}.</font></h1></div>";
    break;
  default: //no error
    $output .= "
          <h1>{$lang_backup['backup_acc']}</h1></div>";
    $output .= "
        <center>
